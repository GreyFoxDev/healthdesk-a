defmodule MainWeb.LocationController do
  ...
  def callback(conn, %{"location_id" => id, "team_id" => team_id, "provider" => provider,"code" => code}=params) do
    res = Ueberauth.Strategy.Google.OAuth.get_access_token [code: code,redirect_uri: "https://staging.healthdesk.ai/admin/teams/#{team_id}/locations/#{id}/#{provider}/callback"]
    IO.inspect("###################")
    IO.inspect(res)
    IO.inspect("###################")
